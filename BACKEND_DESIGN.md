...
public class UpdateSensorDataCommand : IRequest<Result<SensorData>>

// Queries (Read Operations)
public class GetVehiclesQuery : IRequest<Result<List<Vehicle>>>
public class GetFuelAlertsQuery : IRequest<Result<List<FuelAlertDTO>>>
```
...
public class Vehicle : AggregateRoot<VehicleId>
{
    ...
    public FuelCapacity FuelCapacity { get; private set; }
    
    // Domain Methods
    public void UpdateLocation(Location location) { }
    public FuelAlert? CheckFuelLevel(SensorData sensorData) { }
}
```
...
```csharp
// Enmascaramiento automático para usuarios no-admin
public class PrivacyMiddleware
{
    private string MaskId(string id)
    {
        // DEV-****-XC54 format
        return $"DEV-****-{id.Substring(id.Length - 4).ToUpper()}";
    }
}
```
...
public class FleetHub : Hub
{
    ...
    {
        await Clients.Group($"Fleet-{alert.FleetId}").SendAsync("FuelAlert", alert);
    }
}
```
